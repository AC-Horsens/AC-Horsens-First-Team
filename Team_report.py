...
def calculate_expected_points_top_6(df, value_column):
    target_teams = {'Hvidovre', 'Horsens', 'Kolding', 'Esbjerg', 'AaB','Hillerød','Lyngby'}
...
def create_holdsummary(df_possession_stats_summary, df_xg, df_xa, df_matchstats, df_ppda, possession_events):
    ...
    df_possession_stats_summary = pd.melt(
        df_possession_stats_summary, 
        id_vars=['home_team', 'away_team', 'label'], 
        value_vars=['home_possession', 'away_possession'], 
        var_name='possession_type', value_name='terr_poss'
    )
    df_possession_stats_summary['team_name'] = df_possession_stats_summary.apply(
        lambda x: x['home_team'] if x['possession_type'] == 'home_possession' else x['away_team'], axis=1)
    ...
    # --- xG and xA processing ---
    df_xg_hold = df_xg.groupby(['team_name','contestantId', 'label'])['321'].sum().reset_index().rename(columns={'321': 'xG'})
    ...
    zone1_mask = (
        ...
        ((possession_events['x'] > 80) & (possession_events['y'] >= 63) & (possession_events['y'] <= 80)) |
        ((possession_events['x'] > 80) & (possession_events['y'] >= 17) & (possession_events['y'] <= 37))
    )
    ...
    positions = possession_events[['x', 'y']].to_numpy()
    ...
    df_holdsummary = df_holdsummary.merge(df_possession_stats_summary, how='left')
...
def process_data():
    ...
    top_6_teams = ['Hvidovre', 'Horsens', 'Kolding', 'Esbjerg', 'AaB','Hillerød','Lyngby']
...
def create_pdf_game_report(game_data, df_xg_agg, df_xa_agg, merged_df, df_possession_stats, position_dataframes):
    ...
    for _, row in game_merged_df.iterrows():
        ...
        pdf.cell(20, 5, f"{row['Recoveries own third']}", 1)
        pdf.cell(20, 5, f"{row['Recoveries mid third']}", 1)
        pdf.cell(20, 5, f"{row['Recoveries final third']}", 1)
    ...
    pdf.cell(190, 6, txt="Opponent Position Reports", ln=True, align='C')
    pdf.ln(2)
    ...
    for position, df in position_dataframes.items():
        ...
        if 'Total score' in opp_df.columns:
            opp_df['Total score'] = pd.to_numeric(opp_df['Total score'], errors='coerce')


        pdf.set_font("Arial", size=6)
        pdf.cell(190, 4, txt=convert_to_ascii(f"Position Report: {position} (Opponent)"), ln=True, align='C')
        pdf.ln(1)


        # Headers
        headers = opp_df.columns
        col_width = 30
        last_col_width = 15
        if len(headers) > 0:
            for header in headers[:-1]:
                pdf.cell(col_width, 4, txt=convert_to_ascii(header), border=1)
            pdf.cell(last_col_width, 4, txt=convert_to_ascii(headers[-1]), border=1)
            pdf.ln(4)

        # Rows
        for _, r in opp_df.iterrows():
            total_score = r['Total score'] if 'Total score' in opp_df.columns else 0
            if total_score < 4:
                fill_color = (255, 0, 0)
            elif 4 <= total_score <= 6:
                ...
                fill_color = (0, 255, 0)
            pdf.set_fill_color(*fill_color)
            for value in r.values[:-1]:
                pdf.cell(col_width, 4, txt=convert_to_ascii(str(value)), border=1, fill=True)
            pdf.cell(last_col_width, 4, txt=convert_to_ascii(str(r.values[-1])), border=1, fill=True)
            pdf.ln(4)
...
def create_pdf_progress_report(horsens_df, total_expected_points_combined_top_6, position_dataframes):
    ...
    total_expected_points_combined_top_6 = total_expected_points_combined_top_6.sort_values(
        by='Expected points per game', ascending=False
    )
    plt.figure(figsize=(12, 0.01), dpi=500)
    plt.axis('off')
    plt.rc('font', size=6)
    plt.table(cellText=total_expected_points_combined_top_6.values,
              colLabels=total_expected_points_combined_top_6.columns,
...
create_pdf_progress_report(horsens_df, total_expected_points_combined_top_6, position_dataframes)
