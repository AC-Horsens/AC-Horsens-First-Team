...
def League_stats():
    ...
    for _, row in df_short_for_plot_right.iterrows():
        x_start, y_start = row['x'], row['y']
        x_end, y_end = row['140.0'], row['141.0']
        pitch_short_right.arrows(x_start, y_start, x_end, y_end, width=2, headwidth=5, headlength=5, color='yellow', ax=axs[3, 0])

    for _, row in df_short_for_plot_left.iterrows():
        x_start, y_start = row['x'], row['y']
        x_end, y_end = row['140.0'], row['141.0']
        pitch_short_left.arrows(x_start, y_start, x_end, y_end, width=2, headwidth=5, headlength=5, color='yellow', ax=axs[3, 1])
    ...
    plt.tight_layout()
    st.dataframe(df_inswingers_for_plot)
