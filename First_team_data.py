...
@st.cache_data
def load_player_physical_data(league,season):
    url = f'https://raw.githubusercontent.com/AC-Horsens/AC-Horsens-First-Team/main/physical%20data/{league}/{season}_{league}_player_averages.csv'
    physical_data = pd.read_csv(url)
    return physical_data
...
def Dashboard():
    ...
    def set_pieces():
        ...
        df_set_pieces_matches['total_match_xg'] = (
            df_set_pieces_matches.groupby(['label', 'date'])['xG'].transform('sum')
        )
        ...
        Throw_ins = Throw_ins.sort_values(by='Goals_diff',ascending=False)
...
def Opposition_analysis():
    ...
    for col in cols_to_rank:
        if (col == 'PPDA per match') or (col =='xG against per match') or (col=='Transition xG against per match') or (col=='Set piece xG against per match') or (col=='Organized attack xG against per match'):
...
def Physical_data():
    ...
    if main_view == "SS-Team":
        ...
        df = (
            df
            .groupby(["team", "player_name", "position"], as_index=False)
    ...
    # =========================
        ...
        def df_to_pdf_bytes(df: pd.DataFrame, title: str = "WIMU Summary") -> bytes:
            ...
            table.setStyle(TableStyle([
                ...
                ("ROWBACKGROUNDS", (0, 1), (-1, -1), [colors.lightskyblue, colors.white]),
            ]))
