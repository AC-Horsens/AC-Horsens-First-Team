...
def League_stats():
    ...
    def display_team_data(position_df, position_name):
        st.header(position_name)
        
        # Filter the data for 'Horsens' and the 'selected_team'
        horsens_df = position_df[position_df['team_name'] == 'Horsens']
        selected_team_df = position_df[position_df['team_name'] == selected_team]

        # Extract and convert 'match_date' from the 'label' column, dropping null values
        horsens_df['match_date'] = pd.to_datetime(horsens_df['label'].str.extract(r'(\d{4}-\d{2}-\d{2})')[0])
        selected_team_df['match_date'] = pd.to_datetime(selected_team_df['label'].str.extract(r'(\d{4}-\d{2}-\d{2})')[0])

        horsens_df = horsens_df.dropna(subset=['match_date'])
        selected_team_df = selected_team_df.dropna(subset=['match_date'])

        # Use previously calculated 'latest_dates' to filter recent matches for Horsens
        recent_horsens_matches_df = horsens_df[horsens_df['match_date'].isin(latest_dates)]

        # For the selected team, we get the 5 latest match dates and filter accordingly
        latest_selected_team_dates = selected_team_df['match_date'].nlargest(5)
        recent_selected_team_matches_df = selected_team_df[selected_team_df['match_date'].isin(latest_selected_team_dates)]

        # Filter for matches where 'label' contains one of the top 3 similar teams for Horsens only
        matches_with_teams_df_horsens = horsens_df[horsens_df['label'].str.contains('|'.join(teams_list))]
        matches_against_selected_team_df_horsens = horsens_df[horsens_df['label'].str.contains(selected_team)]

        # Combine recent matches and matches involving similar teams for Horsens
        combined_horsens_df = pd.merge(recent_horsens_matches_df, matches_with_teams_df_horsens, how='outer')
        combined_horsens_df = pd.merge(combined_horsens_df, matches_against_selected_team_df_horsens, how='outer')

        # For the selected team, we only use the latest 5 matches
        combined_selected_team_df = recent_selected_team_matches_df

        # Sort by 'Total score' and drop unnecessary columns for both teams
        combined_horsens_df = combined_horsens_df.sort_values(by='Total score', ascending=False).drop(columns=['match_date'])
        combined_selected_team_df = combined_selected_team_df.sort_values(by='Total score', ascending=False).drop(columns=['match_date'])

        # Display the combined DataFrame for Horsens
        st.subheader(f"{position_name} - Horsens Players")
        st.dataframe(combined_horsens_df, hide_index=True)

        # Display the combined DataFrame for the selected team
        st.subheader(f"{position_name} - {selected_team} Players")
        st.dataframe(combined_selected_team_df, hide_index=True)

        # Drop the 'label' column for aggregation and group by player and team for both teams
        combined_horsens_df = combined_horsens_df.drop(columns=['label'])
        combined_selected_team_df = combined_selected_team_df.drop(columns=['label'])

        agg_horsens_df = combined_horsens_df.groupby(['playerName', 'team_name']).agg({
            'minsPlayed': 'sum',
            **{col: 'mean' for col in combined_horsens_df.columns if col not in ['minsPlayed', 'playerName', 'team_name']}
        }).reset_index()

        agg_selected_team_df = combined_selected_team_df.groupby(['playerName', 'team_name']).agg({
            'minsPlayed': 'sum',
            **{col: 'mean' for col in combined_selected_team_df.columns if col not in ['minsPlayed', 'playerName', 'team_name']}
        }).reset_index()

        # Sort by 'Total score' in descending order for both teams
        agg_horsens_df = agg_horsens_df.sort_values(by='Total score', ascending=False)
        agg_selected_team_df = agg_selected_team_df.sort_values(by='Total score', ascending=False)

        # Display the aggregated DataFrame for both teams
        st.subheader(f"Aggregated {position_name} - Horsens Players")
        st.dataframe(agg_horsens_df, hide_index=True)

        st.subheader(f"Aggregated {position_name} - {selected_team} Players")
        st.dataframe(agg_selected_team_df, hide_index=True)

    # Call the function for each position with the corresponding DataFrame
    display_team_data(balanced_central_defender_df, 'Central defenders')
    display_team_data(fullbacks_df, 'Fullbacks')
    display_team_data(number6_df, 'Number 6')
    display_team_data(number8_df, 'Number 8')
    display_team_data(number10_df, 'Number 10')
    display_team_data(winger_df, 'Winger')
    display_team_data(classic_striker_df, 'Striker')
